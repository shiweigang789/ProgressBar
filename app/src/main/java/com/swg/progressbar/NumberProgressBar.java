...
public class NumberProgressBar extends View {
    ...
    private int progressFailColor = 0xFFCACFD9;
    /**
     * 进度条画笔的宽度
     */
    ...
    private int defaultPadding = dp2px(6);
//
//    /**
//     * 三角形的高
//     */
...
//    private Path path = new Path();
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        progressFailColor = attributes.getColor(R.styleable.HorizontalProgressBar_progress_fail_color, Color.parseColor("#CACFD9"));
        bgColor = attributes.getColor(R.styleable.HorizontalProgressBar_progress_color, Color.parseColor("#E7ECFA"));
        progressWidth = attributes.getDimensionPixelSize(R.styleable.HorizontalProgressBar_progress_width, dp2px(6));
        tipRectColor1 = attributes.getColor(R.styleable.HorizontalProgressBar_text_bg_color1, Color.parseColor("#2A5BFF"));
        tipRectColor2 = attributes.getColor(R.styleable.HorizontalProgressBar_text_bg_color2, Color.parseColor("#CACFD9"));
        textColor1 = attributes.getColor(R.styleable.HorizontalProgressBar_text_color1, Color.parseColor("#FFFFFF"));
...
//        mViewHeight = tipRectHeight + triangleHeight + progressWidth + progressMarginTop;
        mViewHeight = tipRectHeight + defaultPadding;
    }
    ...
    private void updatePaintColor() {
        if (mProgress == 0 || mProgress >= 100 || mType == Type.CANCEL) {
            progressPaint.setColor(progressFailColor);
            tipPaint.setColor(progressFailColor);
        } else {
    ...
    private void drawRoundRectAndText(Canvas canvas) {
        float left = currentProgress;
        if (left < tipRectWidth / 2) {
            left = 0;
        } else if (left > mWidth - tipRectWidth / 2) {
            left = mWidth - tipRectWidth;
        } else {
            left = left - tipRectWidth / 2;
        }

        rectF.set(left, getPaddingTop() + defaultPadding, left + tipRectWidth, tipRectHeight);
        ...
        textRect.left = (int) (left + tipRectWidth / 2 - textWidth / 2);
        textRect.top = tipRectHeight / 2 - textHeight / 2 + getPaddingTop() + defaultPadding / 2;
        textRect.right = (int) (left + tipRectWidth / 2 + textWidth / 2);
        textRect.bottom = tipRectHeight / 2 + textHeight / 2 + getPaddingTop() + defaultPadding / 2;
     ...
     * @return
     */
    public void setCurrentProgress(float progress, Type type) {
        mType = type;
    ...
    public void setCurrentProgress(float progress) {
        setCurrentProgress(progress, Type.NORMAL);
    }
     ...
     * @return
     */
    public NumberProgressBar setProgressListener(ProgressListener listener) {
